 ...
 *
 */
public class CopyOfPushTestPage extends WebPage {
	...
	public CopyOfPushTestPage() {
		...
		add(new AjaxLink<Void>("button") {
			...
			public void onClick(AjaxRequestTarget target) {
				status = "working...";
				target.add(CopyOfPushTestPage.this.get("status"));
				final WebSocketConnectionIdentifier connectionId = new WebSocketConnectionIdentifier(this);
				new Timer().schedule(new TimerTask() {
					@Override
					public void run() {
						status = "done!";
						((Image)CopyOfPushTestPage.this.get("image")).setImageResourceReference(new PackageResourceReference(Dummy.class, "england.png"));
		...
		add(new WebSocketPushBehavior() {
			...
			protected void onPush(WebSocketRequestHandler webSocketRequestHandler, IWebSocketPushMessage message) {
				if (message instanceof MyMessage) {
					webSocketRequestHandler.add(CopyOfPushTestPage.this.get("status"));
					webSocketRequestHandler.add(CopyOfPushTestPage.this.get("image"));
				}
			}
