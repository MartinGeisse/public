...
/**
 * Cache implementation for a multi-row, transformed cache. That is, each
 * cache value is generated by the subclass from a list of rows fetched
 * for the same cache key. The value may be or contain a list of values, each
 * generated from a single row, but the cached value may also contain global
 * values generated from all rows for the same cache key.
 * 
