...
package name.martingeisse.forum.application.tools;

import name.martingeisse.forum.application.Configuration;
import name.martingeisse.forum.application.ForumCassandra;
import name.martingeisse.forum.application.Initializer;
...
public final class CassandraReset {
	...
	public CassandraReset() {
		cluster = ForumCassandra.cluster;
		session = ForumCassandra.session;
	}
	...
	public void run() {
		...
		try {
			session.execute("DROP KEYSPACE forum;");
		} catch (InvalidQueryException e) {
			// may happen if the keyspace didn't exist at all
		}
		session.execute("CREATE KEYSPACE forum WITH replication = {'class': 'SimpleStrategy', 'replication_factor': 1};");
		session.execute("USE forum;");
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			ForumCassandra.session.close();
			ForumCassandra.cluster.close();
		}
