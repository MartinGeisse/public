...
public class DoublePageBorder extends Border {
	...
	/**
	 * Constructor.
	 * @param outer the outer border. Must have been created with {@link Constants#PAGE_BORDER_ID}.
	 * @param inner the inner border. Must have been created with {@link Constants#PAGE_BORDER_ID}.
	 */
	public DoublePageBorder(WebMarkupContainer outer, WebMarkupContainer inner) {
		super(Constants.PAGE_BORDER_ID);
		addToBorder(outer);
		outer.add(inner);
	}
	...
	public static WebMarkupContainer combineBorders(WebMarkupContainer[] borders) {
		if (borders.length == 0) {
			return new WebMarkupContainer(Constants.PAGE_BORDER_ID);
