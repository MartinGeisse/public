...
import name.martingeisse.esdk.simulation.OperatingSystemSelector;
...
public final class NativeLibraryHelper {
	 ...
	 * the logger
	 */
	private static Logger logger = Logger.getLogger(NativeLibraryHelper.class);
	...
	private static void extractFile(String name) throws IOException {
		...
		try (InputStream inputStream = NativeLibraryHelper.class.getClassLoader().getResourceAsStream(fullResourceName)) {
			if (inputStream == null) {
