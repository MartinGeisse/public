 ...
 */
public class StackerspaceProtocolClient extends StackdProtocolClient {

	/**
	 * the logger
	 */
	private static Logger logger = Logger.getLogger(StackerspaceProtocolClient.class);
	 ...
	 * Constructor.
	 */
	public StackerspaceProtocolClient() {
		super(IngameHandler.serverName, StackerspaceCommonConstants.NETWORK_PORT);
	}
	...
	public void sendPositionUpdate(ReadableVector3d position, ReadableEulerAngles orientation) {
		StackdPacket packet = new StackdPacket(StackerspacePacketConstants.TYPE_C2S_UPDATE_POSITION, 40);
	...
	protected void onReady() {
		...
		send(new StackdPacket(StackerspacePacketConstants.TYPE_C2S_RESUME_PLAYER, buffer, false));
	...
	protected void onApplicationPacketReceived(StackdPacket packet) {
		...
		case StackerspacePacketConstants.TYPE_S2C_PLAYER_LIST_UPDATE: {
		...
		case StackerspacePacketConstants.TYPE_S2C_PLAYER_NAMES_UPDATE: {
		...
		case StackerspacePacketConstants.TYPE_S2C_PLAYER_RESUMED: {
			synchronized(this) {
				double x = buffer.readDouble();
				double y = buffer.readDouble();
				double z = buffer.readDouble();
		...
		case StackerspacePacketConstants.TYPE_S2C_UPDATE_COINS: {
	...
	public void sendDigNotification(int x, int y, int z) {
		StackdPacket packet = new StackdPacket(StackerspacePacketConstants.TYPE_C2S_DIG_NOTIFICATION, 13);
		ChannelBuffer buffer = packet.getBuffer();
		buffer.writeInt(x);
		buffer.writeInt(y);
		buffer.writeInt(z);
