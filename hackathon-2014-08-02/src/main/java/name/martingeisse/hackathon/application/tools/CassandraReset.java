...
package name.martingeisse.hackathon.application.tools;

import name.martingeisse.hackathon.application.ApplicationCassandra;
import name.martingeisse.hackathon.application.Configuration;
import name.martingeisse.hackathon.application.Initializer;
...
public final class CassandraReset {
	...
	public CassandraReset() {
		cluster = ApplicationCassandra.cluster;
		session = ApplicationCassandra.session;
	}
	...
	public void run() {
		...
		try {
			session.execute("DROP KEYSPACE hackathon_2014_08_02;");
		} catch (InvalidQueryException e) {
			// may happen if the keyspace didn't exist at all
		}
		session.execute("CREATE KEYSPACE hackathon_2014_08_02 WITH replication = {'class': 'SimpleStrategy', 'replication_factor': 1};");
		session.execute("USE hackathon_2014_08_02;");
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			ApplicationCassandra.session.close();
			ApplicationCassandra.cluster.close();
		}
